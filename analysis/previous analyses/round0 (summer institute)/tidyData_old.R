...
evidence_frames <- evidence_frames %>% 
  rename(respFrame = response_frame,
         targetIdx = target,
         noise1frames_direct = noise1frames,
         signal1frames_direct = signal1frames,
         noise2frames_direct = noise2frames) %>%
...
#### make version where noise frames are pulled from evidenceFrames.csv
inference_df_ev <- inference_df %>%
  mutate(congruent = factor(as.character(congruent), levels=c("0", "NaN", "1"), labels=c('incongruent', 'neutral', 'congruent'))) %>%
  left_join(evidence_frames, by=c('subID', 'trial', 'catch_trial', 'congruent', 'respFrame', 'targetIdx', 'response')) %>%
  filter(is.na(noise1frames_direct)==F) %>%
  mutate(signal2frames = ((9*60) - respFrame),
         totalNoise = noise1frames_direct + noise2frames_direct,
         cueIdx = case_when(targetIdx == 1 & congruent == 'congruent' ~ 1,
                            targetIdx == 1 & congruent == 'incongruent' ~ 2,
                            targetIdx == 1 & congruent == 'incongruent' ~ 2,
                            targetIdx == 2 & congruent =='congruent' ~ 2,
                            targetIdx == 2 & congruent=='incongruent' ~ 1,
                            congruent == 'neutral' ~ 3),
         firstNoiseRT = ifelse(respFrame <= noise1frames_direct, 1, 0),
         secondNoiseRT = ifelse(respFrame > (noise1frames_direct+signal1frames_direct) & respFrame <= (noise1frames_direct+signal1frames_direct+noise2frames_direct),
                                1, 0),
         firstSignalRT = ifelse(respFrame > noise1frames_direct & respFrame <= (noise1frames_direct+signal1frames_direct), 1, 0),
         secondSignalRT = ifelse(respFrame > (noise1frames_direct + signal1frames_direct + noise2frames_direct), 1, 0),
         respPeriod = case_when(firstNoiseRT==1 ~ 'noise1',
                                firstSignalRT==1 ~ 'signal1',
                                secondNoiseRT==1 ~ 'noise2',
                                secondSignalRT==1 ~ 'signal2'),
         firstNoise_quartile = cut(noise1frames_direct, breaks=quantile(noise1frames_direct), include.lowest = T, labels=F, na.rm=T), 
         secondNoise_quartile = cut(noise2frames_direct, breaks=quantile(noise2frames_direct), include.lowest = T, labels=F, na.rm=F), 
         totalNoise_quartile = cut(totalNoise, breaks=quantile(totalNoise), include.lowest = T, labels=F, na.rm=T),
         trueAccuracy = case_when(rowSums(across(c(signal1_target1, signal2_target1)), na.rm=T) > rowSums(across(c(signal1_target2, signal2_target2)), na.rm=T) & response==1 ~ 1,
         ...
         accuracyFactor = factor(trueAccuracy, levels=c(1,0), labels=c('correct', 'incorrect')),
         cueCongChoice = case_when(cueIdx==3 ~ NA, cueIdx==response ~ 1, cueIdx!=response ~ 0)) %>% 
  filter(is.na(RT)==F, is.na(confRT)==F) %>% # remove trials with no decision response and trials with no confidence response 
  group_by(subID) %>%
  mutate(logRT = log(RT),
         ...
         rt_quartile = cut(zlogRT, breaks=quantile(zlogRT), include.lowest=T, labels=F),
         trial_quartile = cut(trial, breaks=quantile(trial), include.lowest=T, labels=F)) %>%
  select(-c(block, targetImg, cue_rgb, ))
