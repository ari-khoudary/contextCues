...
def drift(t, signal1_onset, noise2_onset, signal2_onset,
                noise1_cong, signal1_incong, noise2_incong, signal2_incong):
...
try:
    ...
    incong_fitted_params = {k: v for k, v in fitted_params.items() if k.endswith('_incong') or k == 'B' or k == 'noise1_cong'}
    ...
    # get onsets
    incongruent_trials = subject_df[subject_df['trueCongruence'] == 'incongruent']
    onsets = incongruent_trials[['signal1_onset', 'noise2_onset', 'signal2_onset']].value_counts()
    ...
    fitting_model = pyddm.gddm(
        ...
        nondecision=0,
        parameters={'B': (0.5, 12), 
                    'noise1_cong': (0, 10), 'signal1_incong': (0, 10), 'noise2_incong': (0, 10), 'signal2_incong': (0, 10)},
    ...
    generating_model = pyddm.gddm(
        ...
        T_dur = 4.1,
        nondecision=0,
        parameters = incong_fitted_params,
        conditions = ['signal1_onset', 'noise2_onset', 'signal2_onset'])
    ...
    with open(os.path.join(results_dir, f's{subject_id}_incong.txt'), 'w') as f:
        f.write(f'Subject: {subject_id}\nLoss: {loss}\nParameters:\n')
        for param, value in params.items():
            f.write(f'{param}: {value}\n')

    original_stdout = sys.stdout
    with open(os.path.join(results_dir, f's{subject_id}_incong_summary.txt'), "w") as f:
        sys.stdout = f
        fitting_model.show()
