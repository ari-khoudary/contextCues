...
def drift(t, signal1_onset, noise2_onset, signal2_onset,
                noise1_cong, signal1_cong, noise2_cong, signal2_cong):
...
try:
    ...
    cong_fitted_params = {k: v for k, v in fitted_params.items() if k.endswith('_cong') or k == 'B'}
    ...
    # get onsets
    congruent_trials = subject_df[subject_df['trueCongruence'] == 'congruent']
    onsets = congruent_trials[['signal1_onset', 'noise2_onset', 'signal2_onset']].value_counts()
    ...
    fitting_model = pyddm.gddm(
        ...
        nondecision=0,
        parameters={'B': (0.5, 12), 
                    'noise1_cong': (0, 10), 'signal1_cong': (0, 10), 'noise2_cong': (0, 10), 'signal2_cong': (0, 10)},
