...
def make_plots(results_dir, subdir=0, group_only=False):
    ...
    if not group_only:
        for subID in subjects:
            ...
            'Vision-only drift': {params.get('drift').get('fitted'):.3f} [min: {params.get('drift').get('minval'):.3f}, max: {params.get('drift').get('maxval'):.3f}]
            """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--results_dir', type=str, 
                        default='cluster_fitting/visionOnly/results',
