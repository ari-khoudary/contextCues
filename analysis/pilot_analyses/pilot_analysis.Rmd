...
inferenceData <- inference_df %>%
  filter(subID != 16) %>% # remove sub with no noise frame data, and subs with correct noise frame data
...
```{r display RT plots, echo=F, fig.width=8, fig.height=6, fig.dpi=300}
...
```{r display cue*noise1 plots, echo=F, fig.width=8, fig.height=6, dpi=300}
hp + mp + plot_layout(guides='collect')
```
...
# compute expected marginal means
emm <- emmeans(hm, ~ noise2frames_behav | signal1_cueEv, cov.reduce=F)

# quick plot
# emmip(hm,  ~ noise2frames_behav | signal1_cueEv, CIs=T, plotit=T,
#       at = list(noise2frames_behav = c(75, 90, 120, 150), signal1_cueEv = c(-8, 0, 14)))
...
hp <- inferenceData_tidy %>%
  ...
  #geom_ribbon(aes(xvar, yvar, ymin=LCL, ymax=UCL, fill=signal1_cueEv), emm_df, color=NA, alpha=0.5) +
...
emm <- emmeans(mm, ~ noise2frames_behav | signal1_vizEv, 
               at = list(noise2frames_behav = c(75, 90, 120, 150), signal1_vizEv = c(-20, 0, 20)))

# quick plot
# emmip(mm,  ~ noise2frames_behav | signal1_vizEv, CIs=T, plotit=T,
#       at = list(noise2frames_behav = c(75, 90, 120, 150), signal1_vizEv = c(-8, 0, 14)))

# save plot-formatted emmeans
emm_df <- emmip(mm,  ~ noise2frames_behav | signal1_vizEv, CIs=T, plotit=F,
      at = list(noise2frames_behav = c(75, 90, 120, 150), signal1_cueEv = c(-20, 0, 20)))
...
plot_vals <- df %>%
  ...
  summarise(noise_vals = quantile(noise2frames_behav, probs = seq(0,1,0.25), na.rm=T, names=F),
            signal_vals = quantile(signal1_cueEv, probs = seq(0,1,0.25), na.rm=T, names=F))

# compute expected marginal means
emm <- emmeans(hm, ~ noise2frames_behav | signal1_cueEv, type='response', 
               at = list(noise2frames_behav = plot_vals$noise_vals,
                         signal1_cueEv = unique(plot_vals$signal_vals)))
...
hp <- df %>% 
  ...
  geom_errorbar(aes(xvar, yvar, ymin=LCL, ymax=UCL, color=signal1_cueEv), emm_df) +
...
#### Effect of noise2 duration on confidence in noise2 decisions
```{r noise2_conf ~ signal1ev + noise2frames: human, echo=F}

# fit model
hm <- inferenceData_tidy %>%
    filter(catch_trial==0) %>%
    filter(respPeriod == 'noise2', cueIdx < 3) %>%
    # fit model
    lm(zConf ~ noise2frames_behav * signal1_cueEv, .)

# compute expected marginal means
emm <- emmeans(hm, ~ noise2frames_behav | signal1_cueEv, 
               at = list(noise2frames_behav = c(75, 100, 120),
                         signal1_cueEv = c(-9, 2, 13)))
...
# quick plot
# emmip(hm, signal1_cueEv ~ noise2frames_behav, type='response', CIs=T,
...
hp <- inferenceData_tidy %>%
  ...
  # add emmeans
  geom_line(aes(xvar, yvar, color=signal1_cueEv, group=signal1_cueEv), emm_df, linewidth=0.5) +
  labs(title = 'human data', subtitle='signal2 confidence on 80% cue trials only')

# emmeans(hm, eff ~ noise2frames_behav | signal1_cueEv, type='response', cov.reduce=range)
```

#### Effect of noise2 duration on confidence in signal2 decisions
```{r conf ~ signal1ev + noise2frames: human, echo=F}
...
hp <- inferenceData_tidy %>%
  ...
  geom_point(aes(color=signal1_cueEv), size=1.5, position=position_jitter(height=0.1)) + 
  # add emmeans
  geom_line(aes(xvar, yvar, color=signal1_cueEv, group=signal1_cueEv), emm_df, linewidth=2) +
  labs(title = 'human data', subtitle='signal2 confidence on 80% cue trials only') +
  xlim(75, 150)
...
julia_df %>%
  mutate(choose2 = ifelse(response==2, 1, 0)) %>%
  ggplot(aes(x=cueIdx, y=choose2)) + 
  theme_bw() +
  geom_col()
...
julia_df %>%
  ...
  ggplot(aes(x=cueIdx, y=choose1)) + 
  theme_bw() +
  geom_col()

julia_df %>%
  mutate(choose1 = ifelse(response==1, 1, 0)) %>%
  ggplot(aes(x=cueIdx, y=accuracy)) + 
  theme_bw() +
...
inferenceData_tidy %>%
  filter(subID==22) %>%
  ggplot(aes(x=cueIdx, y=cueCongChoice)) +
  stat_summary(fun='mean', geom='bar') + 
  geom_hline(yintercept = 0.5)
