...
### model data
```{r zlogRT ~ noise1frames + cueType: model}
...
### human data
```{r vizLocked_sig2Resp ~ noise2frames_behav + cueType}
# fit model
m <- inferenceData_tidy %>%
  filter(catch_trial == 0) %>%
...
## effect of noise2 duration + cueType on zlogRT in signal2 
### model data
```{r vizLocked_sig2Resp ~ noise2frames_behav + cueType: model}
# fit model
m <- modelData %>%
  filter(respPeriod == 'signal2') %>%
  lm(vizLocked_sig2Resp ~ noise2frames_behav * cueType, ., contrasts = list(cueType = contr.sum))
...
### human data
```{r cueCongChoice_signal1 ~ noise1frames_behav}
...
### model data
```{r cueCongChoice_signal1 ~ noise1frames_behav: model}
...
### human data
```{r cueCongChoice_signal2 ~ noise2frames_behav}
# filter data & fit model
m <- inferenceData_tidy %>%
  filter(catch_trial == 0) %>%
  filter(respPeriod == 'signal2') %>%
...
### model data
```{r cueCongChoice_signal2 ~ noise2frames_behav: model}
# filter data & fit model
m <- modelData %>%
...
### human data
```{r cueCongChoice_signal2 ~ noise2frames_behav * signal1Ev - continuous color: human}
# fit model
m <- inferenceData_tidy %>%
    filter(catch_trial==0) %>%
...
### model data
```{r cueCongChoice_signal2 ~ noise2frames_behav * signal1Ev - continuous color: model}
# filter data
m <- modelData %>%
    filter(respPeriod == 'signal2', cue==0.8) %>% 
    # fit model
