...
signal2_test <- inference_df %>% 
  filter(respLabel == c('signal2'), catch_trial==0)
...
p1 <- mb %>%
  emmeans(., ~ noise2frames_obs  | congCue, type='response', 
          at=list(noise2frames_obs=quantile(signal2_test$noise2frames_obs, na.rm=T, probs=seq(0,1,0.1)),
...
mb <- glm(accuracy ~ scale(noise2frames_obs) * congCue * targetEv_signal1 + 
...
p1 <- mb %>%
  emmeans(., ~ noise2frames_obs * targetEv_signal1  | congCue, type='response', 
          at=list(noise2frames_obs=quantile(signal2_test$noise2frames_obs, na.rm=T, probs=seq(0,1,0.25)),
                  targetEv_signal1 = quantile(signal2_test$targetEv_signal1, na.rm=T, probs=seq(0.05,1,0.25)),
                  congCue = unique(signal2_test$congCue))) %>%
  as.data.frame() %>%
  mutate(congCue = factor(congCue)) %>%
  ggplot(aes(x=noise2frames_obs, y=prob, ymin=asymp.LCL, ymax=asymp.UCL, 
...
signal2_test %>% filter(targetEv_signal1==20) %>% group_by(subID) %>% count()
