...
while needToLearn == 1
    ...
    for trial = 1:learnTrialN
        ...
        % write data to file
        if learningRound == 1
            fprintf(learnFile, '\n %i, %i, %i, %s, %i, %s, %i, %i, %i, %.4f, %.4f, %i', ...
                subID, block, trial, imagePath{imgIdx}, imgIdx, mat2str(thisCue), congruent, response, accuracy, learnRT, cueDurations(trial), learningRound);
        else
            fprintf(learnFile2, '\n %i, %i, %i, %s, %i, %s, %i, %i, %i, %.4f, %.4f, %i', ...
                subID, block, trial, imagePath{imgIdx}, imgIdx, mat2str(thisCue), congruent, response, accuracy, learnRT, cueDurations(trial), learningRound);
        end
